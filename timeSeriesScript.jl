...
function ARMA(T::Int64; φ= [0.8, 0.1, -0.1], θ= [1.7, 1.4, .2, 1.], Z::UnivariateDistribution= Normal(0,1))
	...
	return (Xt= X, meta= "ARMA(Φ=$φ, θ=$θ)")	# return a named tuple
end

#	│	this time series has constant autocovariance function
#	╰────────────────────────────────────────────────────────────
function constantACF(T::Int64; ρ= 0.9, Z::UnivariateDistribution= Normal(0,1))
	...
	return (Xt= X, meta= "constant acf: ρ= $ρ")
end
...
function YuleWalker(T::Int64; ρ= [0.1, -0.1], Z::UnivariateDistribution= Normal(0,1))
	...
	return (Xt= X, meta= "Yule–Walker: Φ=$(round.(φ, sigdigits=3)), ψ=$(round(sqrt(ψ²), sigdigits=3)), γ=$(round.(γ, sigdigits=3))")
end


#	│	main: visualize the time series
#	╰────────────────────────────────────────────────────
maxTime= 1000; copies= 3; ts= Tuple{Any, Any}	# ts holds the time series
...
for i= 1:copies
	ts= constantACF(maxTime; ρ= 0.99, Z= distZ)	# ARMA(maxTime; Z= distZ), YuleWalker(maxTime; ρ= [0.9, 0.8]), constantACF(maxTime; ρ= 0.99, Z= distZ)
